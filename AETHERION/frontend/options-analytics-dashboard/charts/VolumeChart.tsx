...
interface VolumeChartProps {
  data: { strike: number; call_volume: number; put_volume: number }[];
  loading?: boolean;
  error?: string;
...
export default function VolumeChart({ data, loading, error, onEvents }: VolumeChartProps) {
  const { theme } = useTheme();
  const options = useMemo(() => {
    if (data.length === 0) return {};

    const strikes = data.map(d => d.strike.toString());
    const callVol = data.map(d => d.call_volume);
    const putVol = data.map(d => d.put_volume);
    ...
    return {
      ...
      series: [
        { name: "Call Volume", type: "bar", data: callVol, itemStyle: { color: isDark ? "#38BDF8" : "#2C5AA0" }, barMaxWidth: 12 },
        { name: "Put Volume", type: "bar", data: putVol, itemStyle: { color: isDark ? "#F87171" : "#D94F4F" }, barMaxWidth: 12 },
      ],
    };
  }, [data, theme]);
    ...
    return (
      <div className="card">
        <div className="component-loading">
          ...
          <span className="component-loading-text">Loading volume data...</span>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="card">
        <div className="component-error">Unable to load volume data.</div>
      </div>
    );
  }
