...
def run_feature_engineering() -> int:
    ...
    # SQLite: explicitly list columns for insert since BY NAME is missing
    cols = df_priced.columns.tolist()
    placeholders = ",".join(["?"] * len(cols))
    col_str = ",".join(cols)
    
    # Write to final table
    df_priced.to_sql("options_enriched", conn.conn, if_exists="append", index=False)
